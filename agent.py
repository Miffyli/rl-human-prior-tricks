...
class LearningAgent():
    ...
    def env_reward_to_agent(self, agent_prev_obs, agent_obs, reward, done, info) -> float:
        """Shape reward
        """
        return reward
    ...
    def store_experience(
        ...
        self.dqn_agent.replay_buffer.add(
            tree.map_structure(lambda o: o[None], obs),
            tree.map_structure(lambda o: o[None], next_obs),
            action[None],
            reward,
            done,
            [{}]
    ...
    def play(
        ...
        while not env.needs_reset() and (termination_callback is None or not termination_callback(env)) and not done:
            ...
            if training and np.random.rand() < (1 - min(1. - 0.1, self.num_network_updates / self.exploration_steps)):
                agent_action = tree.map_structure(lambda x: np.asarray(x), self.agent_action_space.sample())
            elif not training and eval_epsilon > 0 and np.random.rand() < eval_epsilon:
                agent_action = tree.map_structure(lambda x: np.asarray(x), self.agent_action_space.sample())
            else:
            ...
            reward = self.env_reward_to_agent(agent_obs, agent_next_obs, reward, done, info)
            done = self.env_done_to_agent(agent_obs, agent_next_obs, reward, done, info)
